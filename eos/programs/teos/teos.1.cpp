...
struct ArgsLeft {
  ...
  ArgsLeft() {}
  ArgsLeft(std::vector<string> to_pass_further) {
     argc_ = to_pass_further.size();
     arr_ = new char*[argc_];
     for (size_t i = 0; i < to_pass_further.size(); i++) {
       arr_[i] = new char[to_pass_further[i].size() + 1];

#ifdef _MSC_VER
       strcpy_s(arr_[i], to_pass_further[i].size() + 1,
         to_pass_further[i].c_str());
#else
       strcpy(arr_[i], to_pass_further[i].c_str());
#endif
     }
     argv_ = (const char**)arr_;
  }
...
#define IF_ELSE(commandName_, classPrefix) \
  ...
  { \
    classPrefix##Options(argsLeft.argc_, argsLeft.argv_).go(); \
  } \
  else
...
int main(int argc, const char *argv[]) {
  ...
  try
  {
    ...
    argsLeft = ArgsLeft(to_pass_further);
