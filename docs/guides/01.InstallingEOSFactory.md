...
*EOSFactory* is a truly cross-platform tool. The purpose of this tutorial is to demonstrate how to install it on any operating system, including Windows, OSX and Linux.
...
* We also assume that you have successfully installed *EOSIO*. If that's not the case, please refer to [this guide](https://github.com/EOSIO/eos/wiki/Local-Environment). **NOTE:** if you are on Windows, the only option you have is to install *EOSIO* in the Ubuntu bash (supplied by *Windows Subsystem for Linux*), as EOS source code is not compatible with Windows.
