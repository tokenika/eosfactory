...
And then create the reference to an existing contract by supplying an account which will be holding the contract and the path to the folder where the contract is located:

```
contract = Contract(host, "/path/to/your/smart-contract/folder")
```
