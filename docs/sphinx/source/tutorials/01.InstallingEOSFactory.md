...
If the test fails while executing `node.reset() ` with the `string indices must be integers` error, make sure you have EOS `v1.0.7` installed. Also, make sure no other instance of `nodeos` is running on your machine.
