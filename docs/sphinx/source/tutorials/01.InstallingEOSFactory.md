...
If the test fails while executing `reset() ` with the `string indices must be integers` error, make sure you have EOSIO [v1.2.0](https://github.com/EOSIO/eos/releases/tag/v1.2.0) (or higher) installed. Also, make sure no other instance of `nodeos` is running on your machine.

#### Case 4

If `nodeos` fails to work properly make sure the `EOSIO_SHARED_MEMORY_SIZE_MB` variable has been removed properly, as described above.
