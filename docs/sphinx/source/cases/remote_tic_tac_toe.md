...
reset = False
testnet = .LocalTestnet(reset=reset) # cryptolion kylin  
...
These files should be edited rather, than being deleted. However if the testnet is set to be `.LocalTestnet()`, and the local testnet is reset, the contents of them is useles, then remove them:
