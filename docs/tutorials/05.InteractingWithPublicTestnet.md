...
We recommend using the `python3 -m amaxfactory.register_testnet` script with the following two parameters:
...
```bash
python3 -m amaxfactory.register_testnet http://145.239.133.201:8888 myjungle
```
...
We recommend using the `python3 -m amaxfactory.register_testnet` script with the following five parameters:
...
```bash
python3 -m amaxfactory.register_testnet http://88.99.97.30:38888 myjungle -a tcfchehyagd4 5JBbCwe3t6j63yerYmguRVWg7ZVDY3nKXzGYMwkR9y5w4appKhk 5JYZU9xPS54NhnJrmgQWzVXxZCWpzsVUPS3SBZVZnsPUBFtV5YK
```
...
We recommend using the `amaxfactory.register_testnet_via_faucet` script with the following three parameters:
...
```bash
python3 -m amaxfactory.register_testnet_via_faucet http://faucet.cryptokylin.io http://145.239.133.201:9999 mykylin
```
...
```bash
python3 -m amaxfactory.testnets
```
