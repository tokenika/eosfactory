...
Also, if you intend to work with the [CryptoKylin Testnet](https://www.cryptokylin.io/), *EOSFactory* offers a special script for that purpose, as described in *Case 3*.
...
We recommend using the `utils/register_testnet_kylin.py` script with the following three parameters:
...
```
python3 utils/register_testnet_kylin.py http://faucet.cryptokylin.io https://api.kylin.alohaeos.com mykylin
```
...
```
python3 utils/create_workspace.py foo_bar 03_tic_tac_toe
```
...
Switch to the `foo_ba` folder and first execute `unittest1.py ` without any parameters, just to make sure everything works fine on a local testnet:
...
```
python3 test/unittest1.py kylin
```
