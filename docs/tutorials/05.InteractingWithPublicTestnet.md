...
To be able to create a new account on a public testnet you need an account name and two public keys (owner & active). The problem with account names is that they need to be unique and also comply with strict rules (i.e. `[a-z1-5]{12}` ).
...
```bash
python3 -m eosfactory.create_project foo_bar tic_tac_toe
```
...
**NOTE:** As of now only the `tic_tac_toe` template supports the public testnet interactions.
