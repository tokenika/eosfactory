...
>**NOTE:** The name `master` is just a name of a variable, not the actual name of the account created on the blockchain. The actual (physical) name of the account is `repr(master)`.
...
>**NOTE:** If you don't specify the `account_name` parameter in `create_account()` which is the third parameter, a random name is applied, which is useful on a public testnet where extra restrictions apply. You can see it: `repr(charlie)`
