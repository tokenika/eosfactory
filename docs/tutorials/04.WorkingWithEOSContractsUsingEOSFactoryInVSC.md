...
```bash
cd amaxfactory
python3 -m amaxfactory.create_project foo_bar hello_world
```
...
```bash
cd amaxfactory
python3 -m amaxfactory.create_project foo_bar hello_world --vsc
```
...
The directory of the contract workspace can be seen and set with the command `python3 -m amaxfactory.config --workspace`.
