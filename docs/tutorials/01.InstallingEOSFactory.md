...
Also, if you're on Ubuntu make sure there are no similar export statements in the `~/.bashrc` file. If here are any, remove all of them.
...
```
Especially this one is important - it should return an empty value:

```
echo $EOSIO_SHARED_MEMORY_SIZE_MB
```

Then run the `build.sh` script with the `-r` option:

```
cd eosfactory
./build.sh -e /path/to/your/local/EOSIO/repository -w /path/to/your/local/workspace -r
```
