...
```bash
git clone https://github.com/tokenika/amaxfactory.git
```

Open a bash terminal and navigate to the `amaxfactory` folder:
...
Open a WSL bash terminal and navigate to the `amaxfactory` folder:

```bash
cd /mnt/c/Workspaces/EOS/amaxfactory
```
...
{
    "CONFIG_FILE": "/mnt/c/Workspaces/EOS/amaxfactory/config/config.json",
    "EOSIO_CDT_VERSION": [
        "1.6.1"
    ],
...
Now, you can try tests. Make sure your working directory is switched to `amaxfactory`:
...
```bash
pip3 install --user amaxfactory-tokenika
```
...
Make sure your working directory is switched to `amaxfactory`:
...
The command `python3 -m amaxfactory.config` attempts to verify the version of the EOSIO installation, which is quite frequently unsuccessful and results in the following error message:
...
The current configuration of EOSFactory:
{
    ...
    "AMAX_KEY_PRIVATE": "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3",
    "AMAX_KEY_PUBLIC": "AM6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
    "EOSIO_SHARED_MEMORY_SIZE_MB": "300",
    "EOSIO_VERSION": [
        "1.8.0"
    ],
    "EOS_FACTORY_DIR": "/mnt/c/Workspaces/EOS/amaxfactory",
    "INCLUDE": "/mnt/c/Workspaces/EOS/amaxfactory/includes",
    "KEOSD_EXECUTABLE": "keosd",
    "KEOSD_WALLET_DIR": "/home/cartman/eosio-wallet/",
    "LIBS": null,
    "LOCAL_NODE_ADDRESS": "127.0.0.1:8888",
    "LOCAL_NODE_CONFIG_DIR": "/mnt/c/Workspaces/EOS/amaxfactory/config",
    "LOCAL_NODE_DATA_DIR": "/home/cartman/.local/share/eosio/nodeos/data/",
    "LOCAL_NODE_EXECUTABLE": "nodeos",
    "NODEOS_STDOUT": null,
    "TEMPLATE_DIR": "/mnt/c/Workspaces/EOS/amaxfactory/templates/contracts",
...
file located here:
/mnt/c/Workspaces/EOS/amaxfactory/config/config.json
...
/usr/bin/eosio-cpp -I=/mnt/c/Workspaces/EOS/amaxfactory/contracts_windows/_wslqwjvacdyugodewiyd /mnt/c/Workspaces/EOS/amaxfactory/contracts_windows/_wslqwjvacdyugodewiyd/src/_wslqwjvacdyugodewiyd.cpp:
WASM file writen to file:
    /mnt/c/Workspaces/EOS/amaxfactory/contracts_windows/_wslqwjvacdyugodewiyd/build/_wslqwjvacdyugodewiyd.wasm
######## command line sent to cleos:
/usr/bin/cleos --url http://127.0.0.1:8888 set contract kswquviprmbi /mnt/c/Workspaces/EOS/amaxfactory/contracts_windows/_wslqwjvacdyugodewiyd build/_wslqwjvacdyugodewiyd.wasm build/_wslqwjvacdyugodewiyd.abi
```
Note that there is another facility, corresponding to the `amaxfactory.core.setup.is_print_command_lines` setting: it is `amaxfactory.core.setup.is_save_command_lines`. If set, all the command lines are printed to the file `command_lines.txt` in the current working directory.
