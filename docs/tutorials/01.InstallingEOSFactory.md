...
>**NOTE:** We propose installing the EOSFactory package locally. It installs itself  globally, as well. If it is global, the tests has to be executed as `sudo`. However, if then `KEOSD` has its wallet account local (`$HOME/eosio-wallet`), it issues a misleading fatal error message, something like `Wallet ``_127_0_0_1_8888_default`` already exists.`. 
