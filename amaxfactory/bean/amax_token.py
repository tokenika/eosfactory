...
class AMAX_TOKEN(CreateAccount):
	def __init__(self,contract_name="amax.token"):
		...
		amax_token = new_account(master,contract_name)
		smart = Contract(amax_token, 
			wasm_file=os.getenv("FACTORY_DIR") + "/templates/wasm/" + "amax/amax.token/amax.token.wasm",
			abi_file=os.getenv("FACTORY_DIR") + "/templates/wasm/" + "amax/amax.token/amax.token.abi")
		smart.deploy()
		self = amax_token
	...
	def blacklist(self,targets=[],to_add='true',suber="admin",expect_asset=True):
		self.pushaction("blacklist",{"targets":targets,"to_add":to_add,},suber,expect_asset=expect_asset) 
