...
class MDAO_CONF(CreateAccount):
	...
	def deltag(self,tag='x',submitter_="admin",expect_asset=True):
		self.pushaction("deltag",{"tag":tag,},submitter_,expect_asset=expect_asset) 
	...
	def settag(self,tag='x',submitter_="admin",expect_asset=True):
		self.pushaction("settag",{"tag":tag,},submitter_,expect_asset=expect_asset) 
	...
	def get_global2(self,scope):
		return self.table("global2",scope).json
