...
EOSFACTORY_DIR = "amaxfactory/"
TMP = "/tmp/amaxfactory/"
SETUPTOOLS_NAME = "amaxfactory_tokenika"
# EOSIO_CDT_PATTERN = r".+/amax\.cdt/(\d\.\d\.\d)/.*"
EOSIO_CDT_PATTERN = r".+/amax\.cdt"
...
amaxfactory_data_ = ("EOSFACTORY_DATA_DIR",
...
def amaxfactory_data():
    ...
    if not is_not_linked:
        ...
        for path in amaxfactory_data_[1]:
            tested.append(path)
            if os.path.exists(os.path.join(path, "config", "config.ini")):
                return path
    elif is_not_linked == 2:
        for path in amaxfactory_data_[2]:
            tested.append(path)
            if os.path.exists(os.path.join(path, "config", "config.ini")):
                return path
...
def is_site_package():
    ...
    amaxfactory_path = amaxfactory.__path__
    for item in amaxfactory_path:
        ...
        if "amaxfactory/amaxfactory" in item:
...
# ModuleNotFoundError: No module named 'amaxfactory'

# developer's installation:
# >>> import amaxfactory
# >>> print(amaxfactory.__path__)
# ['/mnt/c/Workspaces/EOS/amaxfactory/amaxfactory']

    if is_local_or_system == -1:
        raise errors.Error('''
Cannot determine the configuration directory. 'amaxfactory.__path__' is
    {}
        '''.format(amaxfactory_path), translate=False)
...
def config_dir():
    path = os.path.join(amaxfactory_data(), CONFIG_DIR)
...
def template_dir():
    path = os.path.join(amaxfactory_data(), TEMPLATE_DIR[1])
    if not os.path.exists(path):
        raise errors.Error('''
...
def eoside_includes_dir():
    ...
    *INCLUDE* entry in the *config.json* file,
    ...
    if not os.path.isabs(path):
        path = os.path.join(amaxfactory_data(), includes_[1])
    if not os.path.exists(path):
        path = None
    return path
...
def eoside_libs_dir():
    ...
    *LIBS* entry in the *config.json* file,
    ...
    path = libs_[1]
    if not os.path.isabs(path):
        path = os.path.join(amaxfactory_data(), libs_[1])
    if not os.path.exists(path):
        path = None
    return path
...
def eosio_key_private():
    ...
    *AMAX_KEY_PRIVATE* entry in the *config.json* file,
    see :func:`.current_config`.
    '''
...
def eosio_key_public():
    ...
    It may be changed with
    *AMAX_KEY_PUBLIC* entry in the *config.json* file,
    see :func:`.current_config`.
    '''
...
def wsl_root():
    ...
    *WSL_ROOT* entry in the *config.json* file,
    ...
    wsl_root_sh = os.path.join(amaxfactory_data(), wsl_root_sh)
...
def keosd_wallet_dir(raise_error=True):
    ...
    if not path:
        from amaxfactory.core.cleos import WalletList
        WalletList()
        path = first_valid_path(keosd_wallet_dir_, raise_error=False)
        if not path:
            if raise_error:
...
def contract_dir(contract_dir_hint):
    ...
    # 'amaxfactory_data()/contracts'
    contract_dir_ =  os.path.join(
                amaxfactory_data(), CONTRACTS_DIR, contract_dir_hint)
...
def current_config(contract_dir=None, dont_set_workspace=False):
    ...
    Note:
        ...
        *python3 -m amaxfactory.core.config*
    '''
    map = {}
    ...
    try:
        ...
        map[libs_[0]] = None
    try:
        map[amaxfactory_data_[0]] = amaxfactory_data()
    except:
        map[amaxfactory_data_[0]] = None
    try:
