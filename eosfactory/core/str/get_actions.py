...
class GetActions():
    ...
    def __init__(
        self, received_json, 
        as_json=False, full=False, pretty=False, console=False):
        
        if as_json:
            if pretty:
                self.info = json.dumps(received_json, indent=4)
            else:
                self.info = json.dumps(received_json)
            return
...
if __name__ == '__main__':
   print(GetActions(example, as_json=False, pretty=False, console=True))