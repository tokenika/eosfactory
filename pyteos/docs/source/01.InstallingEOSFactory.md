...
**NOTE:**  Regarding Ubuntu, as EOSFactory spawns `gnome-terminal` with a `nodeos` process inside, it's necessary to have `gnome-terminal` and an X server installed.
...
## Build EOSFactory on Linux & MacOS
