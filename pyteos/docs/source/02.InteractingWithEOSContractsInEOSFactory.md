...
In Ubuntu & MacOS the *EOSIO* process is run in the background with no visible interface. However in Windows it's launched in a separate window.
...
Now that `alice` has tokens, we will transfer some to the `carol` account: 
...
To exit Python CLI use the `exit` Python command:
