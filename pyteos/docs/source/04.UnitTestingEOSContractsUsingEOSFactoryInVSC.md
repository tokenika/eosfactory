...
For the time being you can keep your contracts in a separate location and refer to them using a full path, e.g.

```
c = Contract("/mnt/x/Workspaces/contracts/hello")
```

However, when you create a new contract from the template, paths are not currently supported, so you need to use the contract's folder name e.g.
...
Ultimately we want to make unit-testing much better organized and integrated with *Visual Studio Code*.