...
namespace pentagon{
  namespace control{
    ...
    void stopChainNode() 
    {
      string pid;
      string echo;
      string CHAIN_NODE = (pentagon::config::get_EOSIO_INSTALL_DIR() / "bin/eosiod").string();
    ...
    )
    {
      ...
      string commandLine;
      commandLine += (pentagon::config::get_EOSIO_INSTALL_DIR() 
        / "bin/abi_gen").string()
        + " -extra-arg=-c"
        + " -extra-arg=--std=c++14"
        + " -extra-arg=--target=wasm32"
        + " -extra-arg=-I" + (pentagon::config::get_EOSIO_INSTALL_DIR() / "include").string()
    ...
    {
      string commandLine;
      commandLine += pentagon::config::get_WASM_CLANG().string()
        + " --help";
      boostProcessSystem(commandLine);   
    }
    ...
    void buildContract(
      ...
      for(string file : src)
      {
        ...
        commandLine += pentagon::config::get_WASM_CLANG().string()
          ...
          + " -I " + (pentagon::config::get_EOSIO_INSTALL_DIR() / "include").string()
      ...
      {
        string commandLine;
        commandLine += pentagon::config::get_WASM_LLVM_LINK().string()
          + " -o " + linked
          + " " + objectFileList;
        cout << commandLine << endl;
        /* 
      ...
      {
        string commandLine;
        commandLine += pentagon::config::get_WASM_LLC().string()
          + " --asm-verbose=false"
          + " -o " + assembly 
          + " " + linked;
        cout << commandLine << endl;      
      ...
      {
        string commandLine;
        commandLine += (pentagon::config::get_BINARYEN_BIN() / "s2wasm").string()
          + " -o " + targetWastFile
          + " -s 16384"
          + " " + assembly;
        //cout << commandLine << endl;
